...
if __name__ == '__main__': 
    ...
    noise_idx = load_tabular_data('blog', {'train':args.train_num, 'valid':args.valid_num}, noise_rate=args.noise_rate, out=f'{args.out}/{run_id}/data_files/') # saves to disk
    ...
    model = NN(in_channels=280, out_channels=2, num_layers=args.num_layers, hidden_channels=args.hidden_channels, norm=True, dropout=args.do, bias=True, act=torch.nn.Mish, out_fn=torch.nn.Softmax(dim=-1))
    ...
    pred = NN(in_channels=280, out_channels=2, num_layers=args.num_layers, hidden_channels=args.hidden_channels, norm=True, dropout=args.do, bias=True, act=torch.nn.Mish, out_fn=torch.nn.Softmax(dim=-1))
