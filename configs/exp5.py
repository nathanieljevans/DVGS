...
summary="This experiment measures the ability of (4) methods for capturing exogenous noise in the blog dataset (supervised)."
...
model = NN(in_channels=280, out_channels=2, num_layers=2, hidden_channels=100, norm=True, dropout=0.5, bias=True, act=torch.nn.Mish, out_fn=torch.nn.Softmax(dim=-1))
...
estimator = NNEst(xin=280, yin=4, y_cat_dim=50, out_channels=1, num_layers=4, hidden_channels=200, norm=False, dropout=0.0, bias=True, act=torch.nn.ReLU)
